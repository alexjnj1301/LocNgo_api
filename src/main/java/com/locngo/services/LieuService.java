...
public class LieuService {
  @Value("${links.replacement}")
  private String replacementImageLink;

  @Autowired private LieuRepository lieuRepository;
  @Autowired private LieuServicesService lieuServicesService;
  @Autowired private ServicesRepository servicesRepository;
  @Autowired private ReservationService reservationService;
  @Autowired private LieuImageService lieuImageService;
  ...
  public LieuResponseDto findById(int id) {
    ...
    List<LieuImageDto> imagesDto =
        lieu.getImages().stream()
            .map(image -> new LieuImageDto(image.getId(), image.getImageUrl(), null))
            .collect(Collectors.toList());

    List<SimpleServiceDto> servicesDto =
        lieu.getServices().stream()
            .map(
                serviceMapping ->
                    new SimpleServiceDto(
        ...
        .map(
            lieu ->
                new AllLieuResponseDto(
                    ...
                    lieu.getPostal_code(),
                    null,
                    this.getFavoritePicture(lieu.getId()).isEmpty()
                        ? replacementImageLink
                        : this.getFavoritePicture(lieu.getId()),
                    null))
        .collect(Collectors.toList());
  }
  ...
  public CreateLieuDto createLieu(CreateLieuDto lieuDto, String token) {
    ...
    var lieu =
        new Lieu(
            ...
            lieuDto.price(),
            lieuDto.description(),
            lieuDto.favorite_picture(),
            (double) connectedUser.getId(),
            null,
            null,
            null);
    var createdLieu = lieuRepository.save(lieu);
    this.lieuImageService.addImageToLieu(
        new CreateLieuImageDto(0, lieuDto.favorite_picture(), createdLieu));
  ...
  public LieuDto addServicesToLieu(int lieuId, List<Integer> servicesId) {
    var lieu =
        this.lieuRepository
            .findById(lieuId)
            .orElseThrow(() -> new LieuNotFoundException("Lieu not found"));
    servicesId.forEach(
        id -> {
          var services =
              servicesRepository
  ...
  public void setLieuFavoritePicture(SetLieuFavoritePictureDto favoritePictureDto) {
    ...
    var lieu =
        this.lieuRepository
            .findById(favoritePictureDto.lieuId())
            .orElseThrow(
                () ->
                    new LieuNotFoundException(
                        "Lieu with id " + favoritePictureDto.lieuId() + " not found"));
    lieu.getImages().stream()
        .filter(lieuImage -> lieuImage.getId() == favoritePictureDto.imageId())
        .findFirst()
        .orElseThrow(
            () ->
                new NotAllowedToAccessThisResourceException(
